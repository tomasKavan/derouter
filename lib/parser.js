...
const configObjectParser = require('./helper/config-object-parser.js')
...
/**
 * Parse routes configuration on input, creates and returns express js
 * routes instance.
 *
...
function parse(config, formatHint) {
  ...
  if (typeof config === 'string') {
    ...
    config = configObjectParser(config)
  }
  ...
  assembly.forEach(route => {
    ...
    expressRouter[route.method](route.route, route.middleware, fn)
  })
...
/**
 * Compile handler for Express routes
 *
...
function compileHandler(handler) {
  // wrapper - express routes handler
...
function processHandlerException(e, res) {
  ...
  try {
    if(typeof e === 'number'){
      e = {
        code: e,
        text: HttpStatus.getStatusText(e)
      }
    } else if(typeof e.code !== 'number'){
      e = {
        code: 500,
        text: HttpStatus.getStatusText(500)
      }
    }
  } catch (err) {
    console.error(err)
