...
function find(path) {
  if (typeof path !== 'string' && !(path instanceof Array)) {
    throw new Error('Action Finder: path must be an array or dot separated string.')
  }
  ...
  if (pathArr.length < 2) {
    throw new Error('Action Finder: Path must have atleast 2 components.')
  }
  ...
  throw new Error('Action Finder: Action not found')
}
